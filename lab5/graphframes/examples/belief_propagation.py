...
class BeliefPropagation(object):
    ...
    * In each step of BP, update all vertices of a single color.  Alternate colors.
    ...
    def runBPwithGraphFrames(cls, g, numIter):
        ...
        gx = GraphFrame(
            colorG.vertices.withColumn("belief", sqlfunctions.lit(0.0)), colorG.edges
        )
        ...
        for iter_ in range(numIter):
            ...
            for color in range(numColors):
                ...
                newVertices = (
                    v.join(
                        aggregates, on=(v["id"] == aggregates["id"]), how="left_outer"
                    )
                    .drop(
                        aggregates["id"]
    ...
    def _colorGraph(g):
        ...
        colorUDF = sqlfunctions.udf(
            lambda i, j: (i + j) % 2, returnType=types.IntegerType()
        )
        v = g.vertices.withColumn(
            "color", colorUDF(sqlfunctions.col("i"), sqlfunctions.col("j"))
        )
